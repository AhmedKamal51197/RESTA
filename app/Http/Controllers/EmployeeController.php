...
class EmployeeController extends Controller
{
    ...
    public function filterAdmin(Request $request)
    {
        ...
        if ($employees->isEmpty()) {
            return response()->json([
                'status' => 'failed',
                'message' => 'No admins found with the given filters'
            ], 404);
        }
    ...
    public function filterEmployee(Request $request)
    {
        ...
        $filters = $request->only(['name', 'email', 'phone', 'status', 'role']);
        ...
        foreach ($filters as $key => $value) {
            if (!is_null($value)) {
                if ($key === 'name' || $key === 'email' || $key === 'phone') {
                    ...
                    $mappedValue = $roleMapping[$value] ?? null;
                    if ($mappedValue !== null) {
                        $query->where('Role', $mappedValue);
                    }
                }
            }
        }
