...
class OrderController extends Controller
{
    ...
    //filter order
    public function filterTableOrde(Request $request)
    {
        $query = Order::query()
            ->join('customers', 'orders.customer_id', '=', 'customers.id')
            ->select('orders.id', 'orders.status', 'orders.created_at', 'orders.total_cost', 'customers.name as customer');
        
        $filters = $request->only(['id', 'status', 'date']);
        ...
        foreach ($filters as $key => $value) {
            if (!is_null($value)) {
                if ($key == 'status') {
                    ...
                    if ($mappedValue !== null) {
                        $query->where('orders.status', $mappedValue);
                    }
                } elseif ($key == 'customer') {
                    $query->where('customers.name', 'like', '%' . $value . '%');
                } elseif ($key == 'date') {
                    try {
                        $date = \Carbon\Carbon::createFromFormat('m/d/y', $value)->format('Y-m-d');
                    } catch (\Exception $e) {
                        try {
                            $date = \Carbon\Carbon::createFromFormat('m/d/Y', $value)->format('Y-m-d');
                        } catch (\Exception $e) {
                            try {
                                $date = \Carbon\Carbon::createFromFormat('Y-m-d', $value)->format('Y-m-d');
                            } catch (\Exception $e) {
                                return response()->json([
                                    'status' => 'failed',
        ...
        if ($DataOrders->isEmpty()) {
            return response()->json([
                'status' => 'failed',
                'message' => 'No order found with the given filters'
            ], 404);
        }
