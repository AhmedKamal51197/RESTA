    ...
    public function filterOffer(Request $request)
{
    ...
    $filters = $request->only(['name', 'discount', 'start_date', 'end_date', 'status']);
    ...
    // Adjust column names to match your desired output format
    $dataOffers = $query->select('id', 'name', 'discount','status', 'startDate as start_date', 'endDate as end_date')->get();

    if ($dataOffers->isEmpty()) {
        return response()->json([
            'status' => 'failed',
            'message' => 'No offer found with the given filters'
        ], 404);
    }
